...
class testProfessorViews(TestCase):
    ...
    def test_professor_form_view(self):
        ...
        # ? after login
        self.client.login(username="testprofessor", password="test")
        respone_after_login = self.client.get(reverse("academic:register_professor"))
    ...
    def test_professor_profile_view(self):
        response = self.client.get(reverse("academic:professor_profile"))
        ...
        # ? after login
        self.client.login(username="testprofessor", password="test")
        response_after_login = self.client.get(reverse("academic:professor_profile"))
        self.assertEqual(response_after_login.status_code, 200)
    ...
    def test_professor_lesson_list_view(self):
        response = self.client.get(reverse("academic:professor_lessons", kwargs={"p_code":"p_code", "u_code":"u_code"}))
        ...
        for i in professor.universities.all():
            response_after_getting_classes = self.client.get(reverse("academic:professor_lessons", kwargs={"p_code":str(professor.code), "u_code":str(i.code)}))
    ...
    def test_professor_lesson_details(self):
        response = self.client.get(reverse("academic:lesson_detail", kwargs={"l_code":"l_code"}))
        ...
        for i in professor.classes.all():
            response_after_getting_class = self.client.get(reverse("academic:lesson_detail", kwargs={"l_code":i.lesson_code.code}))
    ...
    def test_grade_form_view(self):
        response = self.client.get(reverse("academic:grade", kwargs={"l_code":"l_code", "class_code":1234}))
        self.assertEqual(response.status_code, 302)
